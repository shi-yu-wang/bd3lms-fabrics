...
class Diffusion(nn.Module):
  def __init__(
    ...
    self.metrics.train_nlls = self.metrics.train_nlls.to("cpu")
  ...
  def _subs_parameterization(self, logits, xt):
    ...
    mask3d = unmasked_indices.unsqueeze(-1)
    
    # Create neg_infinity tensor with correct dtype and device
    neg_inf = torch.tensor(self.neg_infinity, dtype=logits.dtype, device=logits.device)
    logits = torch.where(mask3d, neg_inf, logits)
    
    one_hot = torch.nn.functional.one_hot(xt, num_classes=logits.size(-1)).to(torch.bool)
    gt_mask = mask3d & one_hot
    logits = torch.where(gt_mask,
  ...
  def training_step(self, batch, batch_idx):
    ...
    nll   = losses.nlls.detach()
    mask  = losses.token_mask.detach()
    batch_nll = (nll * mask).sum() / mask.sum()      # scalar on TPU
    # Move to CPU before updating metrics
    batch_nll_cpu = batch_nll.cpu()
    xm.mark_step()
    self.metrics.train_nlls.update(batch_nll_cpu.item())
    return losses.loss
  ...
  def _resample_q_xt(
    ...
    """
    x: [batch_size, block_size * num_blocks]
    xt: [batch_size, num_blocks, block_size]
    move_indices: [batch_size, block_size * num_blocks]
    p: [batch_size, block_size * num_blocks]
    ...
    # print(f"self.mask_index: {self.mask_index}") # 50257
    perc_masked = (xt == self.mask_index).float().sum(-1) / block_size # [batch_size, num_blocks]
    # print(f"Initial perc_masked: {perc_masked}")
    prev_perc_masked = None
    recurr = 0
    while (perc_masked < sampling_eps_min).any() or (perc_masked > sampling_eps_max).any():
      recurr += 1
      # print(f"Current perc_masked: {perc_masked}")
      # if prev_perc_masked is not None:
      #   print(f"Is perc_masked unchanged? {torch.allclose(perc_masked, prev_perc_masked)}")
      # print(f"p_mini: {p[:2, [0, 1, 4, 5, 8, 9]]}, Blocks below min: {(perc_masked < sampling_eps_min).sum()}, p min: {p.min()}, recurr: {recurr}, sampling_eps_min: {sampling_eps_min}")
      # print(f"Blocks above max: {(perc_masked > sampling_eps_max).sum()}")
      print(f"blocks to resample: {(perc_masked < sampling_eps_min).sum()}")
      ...
      if sampling_eps_min == 1e-3 and sampling_eps_max != 1:
        ...
        regen_idx = (perc_masked < sampling_eps_min) | (perc_masked > sampling_eps_max) # [batch_size, num_blocks]
      # print(f"_resample_q_xt 1")
      # print(f"Number of blocks to resample: {regen_idx.sum()}")
      regen_idx = regen_idx.repeat_interleave(block_size,dim=-1) # 没mask掉的block全部remask

      # Ensure p is on the correct device
      p = p.to(x.device)
      move_indices[regen_idx] = (torch.rand_like(x) < p)[regen_idx]
      xt = torch.where(move_indices, self.mask_index, x)
      xt = xt.reshape(xt.shape[0], -1, block_size)
      prev_perc_masked = perc_masked.clone()
  ...
  def _sigma_from_p(self, p):
    print(f"p device in _sigma_from_p: {p.device}, type: {p.dtype}")
    result = torch.min(- torch.log(1 - p), self.noise.sigma_max)
    print(f"result device in _sigma_from_p: {result.device}, type: {result.dtype}")
    return result
  ...
  def _forward_pass_diffusion(self, x0, t=None, sampling_eps_min=None, sampling_eps_max=None):
    print(f"x0 device: {x0.device}, type: {x0.dtype}")
    if t is None:
      print(f"Start sample t")
      t = self._sample_t(x0.shape,
                         sampling_eps_min,
                         sampling_eps_max)
    loss_scale, p = self.noise(t) # p = t
    p = p.to(x0.device)
    print(f"noise computed")
    sigma = self._sigma_from_p(p[:,0].unsqueeze(-1))
    print(f"sigma device: {sigma.device}, type: {sigma.dtype}")
    ...
    if self.mdlm_loss_scale:
      ...
      print(f"sigma device after mdlm: {sigma.device}, type: {sigma.dtype}")
